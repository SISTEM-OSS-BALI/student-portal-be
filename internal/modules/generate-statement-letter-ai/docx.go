...
func generateStatementLetterDocument(_ context.Context, input GenerateDTO, content string) (string, string, error) {
	docxBytes, err := buildStatementLetterDocxBytes(input, content)
	if err != nil {
		return "", "", err
	}

	fileName := buildGeneratedStatementLetterFileName(input)
	return base64.StdEncoding.EncodeToString(docxBytes), fileName, nil
}

func buildGeneratedStatementLetterFileName(input GenerateDTO) string {
	name := sanitizeFileName(stringPtrValue(input.StudentName))
	if name == "" {
		name = "student"
	}

	return fmt.Sprintf("%s_Statement_Letter.docx", name)
}

func buildStatementLetterDocxBytes(input GenerateDTO, content string) ([]byte, error) {
	var output bytes.Buffer
	writer := zip.NewWriter(&output)

	files := map[string]string{
		"[Content_Types].xml":          buildStatementLetterContentTypesXML(),
		"_rels/.rels":                  buildStatementLetterRootRelsXML(),
		"word/document.xml":            buildStatementLetterDocumentXML(input, content),
	...
	if err := writer.Close(); err != nil {
		return nil, fmt.Errorf("finalize statement letter docx: %w", err)
	}

	return output.Bytes(), nil
}

func buildStatementLetterContentTypesXML() string {
...
func buildStatementLetterRootRelsXML() string {
	return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
func buildStatementLetterDocumentXML(input GenerateDTO, content string) string {
	title := "Statement Letter"
	if name := strings.TrimSpace(stringPtrValue(input.StudentName)); name != "" {
		title = fmt.Sprintf("Statement Letter - %s", name)
	}
	...
	builder.WriteString(buildStatementLetterParagraphsXML(content))
...
func buildStatementLetterParagraphsXML(content string) string {
	...
	if !hasContent {
		builder.WriteString(`<w:p><w:r><w:t xml:space="preserve">Statement letter content is empty.</w:t></w:r></w:p>`)
	}
