...
func (s *GeneratedDocumentService) Upsert(input GeneratedDocumentUpsertDTO) (schema.GeneratedStatementLetterAIDocument, error) {
	studentID := strings.TrimSpace(input.StudentID)
	fileURL := strings.TrimSpace(input.FileURL)
	if studentID == "" {
		return schema.GeneratedStatementLetterAIDocument{}, errors.New("student_id is required")
	}
	if fileURL == "" {
		return schema.GeneratedStatementLetterAIDocument{}, errors.New("file_url is required")
	}

	doc, err := s.repo.GetByStudentID(studentID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			newDoc := schema.GeneratedStatementLetterAIDocument{
				StudentID:    studentID,
				FileURL:      fileURL,
...
func (s *GeneratedDocumentService) ListByStudentID(studentID string) ([]schema.GeneratedStatementLetterAIDocument, error) {
	return s.repo.ListByStudentID(strings.TrimSpace(studentID))
}

func (s *GeneratedDocumentService) GetByStudentID(studentID string) (schema.GeneratedStatementLetterAIDocument, error) {
