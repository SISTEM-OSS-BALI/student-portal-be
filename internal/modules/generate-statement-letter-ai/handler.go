...
func (h *Handler) Generate(c *gin.Context) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			log.Printf("[statement-letter.generate] panic after=%s panic=%v\n%s", time.Since(start), rec, debug.Stack())
			if !c.Writer.Written() {
				httpx.RespondError(c, http.StatusInternalServerError, "generate_panic", "unexpected error while generating statement letter", nil)
			}
		}
	}()
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		log.Printf("[statement-letter.generate] bind error: %v", err)
	...
	if err != nil {
		log.Printf("[statement-letter.generate] failed after=%s err=%v", time.Since(start), err)
