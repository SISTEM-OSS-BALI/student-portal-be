...
type GeneratedDocumentUpsertDTO struct {
	...
	Status                *schema.StatementLetterDocumentStatus `json:"status"`
...
type GeneratedDocumentApprovalLogDTO struct {
	...
	FromStatus *schema.StatementLetterApprovalStatus `json:"from_status,omitempty"`
	ToStatus   schema.StatementLetterApprovalStatus  `json:"to_status"`
...
type GeneratedDocumentApprovalDTO struct {
	...
	Status     schema.StatementLetterApprovalStatus  `json:"status"`
	Note       *string                               `json:"note,omitempty"`
...
func newGeneratedDocumentApprovalReviewerDTO(user *schema.User) *GeneratedDocumentApprovalReviewerDTO {
	...
	return &GeneratedDocumentApprovalReviewerDTO{
		ID:    user.ID,
		Name:  user.Name,
		Email: user.Email,
		Role:  user.Role,
...
func newGeneratedDocumentApprovalLogDTO(item *schema.StatementLetterAIApprovalLog) *GeneratedDocumentApprovalLogDTO {
	if item == nil {
		return nil
	}
	return &GeneratedDocumentApprovalLogDTO{
		ID:         item.ID,
		ApprovalID: item.ApprovalID,
		ActorID:    item.ActorID,
		Actor:      newGeneratedDocumentApprovalReviewerDTO(item.Actor),
...
func newGeneratedDocumentApprovalDTO(item *schema.StatementLetterAIApproval) *GeneratedDocumentApprovalDTO {
	if item == nil {
		return nil
	}
	response := &GeneratedDocumentApprovalDTO{
		ID:         item.ID,
		DocumentID: item.DocumentID,
		ReviewerID: item.ReviewerID,
		Reviewer:   newGeneratedDocumentApprovalReviewerDTO(item.Reviewer),
		Status:     item.Status,
		Note:       item.Note,
	...
	if len(item.Logs) > 0 {
		...
		for _, logItem := range item.Logs {
			logDTO := newGeneratedDocumentApprovalLogDTO(&logItem)
			if logDTO != nil {
				response.Logs = append(response.Logs, *logDTO)
			}
		}
	}
	return response
...
func NewGeneratedDocumentResponseDTO(doc schema.GeneratedStatementLetterAIDocument) GeneratedDocumentResponseDTO {
	response := GeneratedDocumentResponseDTO{
		...
		ChecklistSource:        statementChecklistSource(),
		SubmittedToDirectorAt:  doc.SubmittedToDirectorAt,
		ApprovedAt:             doc.ApprovedAt,
		RevisionRequestedAt:    doc.RevisionRequestedAt,
		CurrentApprovalID:      doc.CurrentApprovalID,
		CreatedAt:              doc.CreatedAt,
		UpdatedAt:              doc.UpdatedAt,
	}
	if doc.Status == schema.StatementLetterDocumentStatusApproved {
		response.CanDownloadPDF = true
		downloadURL := "/api/generate-statement-letter-ai/documents/" + doc.ID + "/download-pdf"
	...
	if len(doc.Approvals) > 0 {
		...
		for _, item := range doc.Approvals {
			approvalDTO := newGeneratedDocumentApprovalDTO(&item)
			if approvalDTO != nil {
				response.Approvals = append(response.Approvals, *approvalDTO)
			}
		}
	}
	return response
...
func NewGeneratedDocumentResponseListDTO(docs []schema.GeneratedStatementLetterAIDocument) []GeneratedDocumentResponseDTO {
