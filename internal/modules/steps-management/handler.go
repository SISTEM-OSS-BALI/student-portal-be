...
func (h *Handler) Create(c *gin.Context) {
	...
	step, err := h.service.Create(input)
	if err != nil {
		httpx.RespondError(c, http.StatusBadRequest, "create_failed", err.Error(), nil)
		return
	}

	c.JSON(http.StatusCreated, NewResponseDTO(step))
}

func (h *Handler) List(c *gin.Context) {
	steps, err := h.service.List()
	if err != nil {
		httpx.RespondError(c, http.StatusInternalServerError, "list_failed", err.Error(), nil)
		return
	}
	c.JSON(http.StatusOK, NewResponseListDTO(steps))
}

func (h *Handler) GetByID(c *gin.Context) {
	step, err := h.service.GetByID(c.Param("id"))
	if err != nil {
		httpx.RespondError(c, http.StatusNotFound, "not_found", "step not found", nil)
		return
	}
	c.JSON(http.StatusOK, NewResponseDTO(step))
}
...
func (h *Handler) Update(c *gin.Context) {
	...
	step, err := h.service.Update(c.Param("id"), input)
	if err != nil {
