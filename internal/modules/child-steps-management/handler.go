...
func (h *Handler) Create(c *gin.Context) {
	...
	child, err := h.service.Create(input)
	if err != nil {
		httpx.RespondError(c, http.StatusBadRequest, "create_failed", err.Error(), nil)
		return
	}

	c.JSON(http.StatusCreated, NewResponseDTO(child))
}

func (h *Handler) List(c *gin.Context) {
	children, err := h.service.ListWithSteps()
	if err != nil {
		httpx.RespondError(c, http.StatusInternalServerError, "list_failed", err.Error(), nil)
		return
	}
	c.JSON(http.StatusOK, NewResponseListDTO(children))
}

func (h *Handler) GetByID(c *gin.Context) {
	child, err := h.service.GetByID(c.Param("id"))
	if err != nil {
		httpx.RespondError(c, http.StatusNotFound, "not_found", "child step not found", nil)
		return
	}
	c.JSON(http.StatusOK, NewResponseDTO(child))
}
...
func (h *Handler) Update(c *gin.Context) {
	...
	child, err := h.service.Update(c.Param("id"), input)
	if err != nil {
