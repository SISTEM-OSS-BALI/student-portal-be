...
func (h *Handler) CreateOrUpdate(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, ErrStatementLetterNotSubmitted):
			statusCode = http.StatusConflict
		}
		httpx.RespondError(c, statusCode, "statement_letter_ai_approval_upsert_failed", err.Error(), nil)
		return
	}
...
func (h *Handler) List(c *gin.Context) {
	...
	if err != nil {
		httpx.RespondError(c, http.StatusInternalServerError, "statement_letter_ai_approval_list_failed", err.Error(), nil)
		return
	}
...
func (h *Handler) GetByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			httpx.RespondError(c, http.StatusNotFound, "statement_letter_ai_approval_not_found", "statement letter ai approval not found", nil)
			return
		}
		httpx.RespondError(c, http.StatusInternalServerError, "statement_letter_ai_approval_get_failed", err.Error(), nil)
		return
	}
	c.JSON(http.StatusOK, NewResponseDTO(item))
}
...
func (h *Handler) Update(c *gin.Context) {
	...
	if err != nil {
		...
		httpx.RespondError(c, statusCode, "statement_letter_ai_approval_update_failed", err.Error(), nil)
		return
	}
