...
type CreateOrUpdateDTO struct {
	...
	Status     schema.StatementLetterApprovalStatus `json:"status" binding:"required"`
...
func newDocumentSummaryDTO(doc *schema.GeneratedStatementLetterAIDocument) *DocumentSummaryDTO {
	if doc == nil {
		return nil
	}
...
func newLogResponseDTO(log schema.StatementLetterAIApprovalLog) LogResponseDTO {
	return LogResponseDTO{
		ID:         log.ID,
		ApprovalID: log.ApprovalID,
		ActorID:    log.ActorID,
		Actor:      newReviewerDTO(log.Actor),
...
func NewResponseDTO(item schema.StatementLetterAIApproval) ResponseDTO {
	...
	return ResponseDTO{
		...
		Reviewer:   newReviewerDTO(item.Reviewer),
		Document:   newDocumentSummaryDTO(item.Document),
		Status:     item.Status,
		Note:       item.Note,
...
func NewResponseListDTO(items []schema.StatementLetterAIApproval) []ResponseDTO {
